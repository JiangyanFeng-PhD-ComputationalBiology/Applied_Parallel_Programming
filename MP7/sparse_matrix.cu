...
__global__ void spmvJDSKernel(float *out, int *matColStart, int *matCols,
  ...
  if (row < dim){
    float dot = 0;
    int ele = 0;
    while (ele < matRows[row]){
      int row_idx = matColStart[ele] + row;
      int col_idx = matCols[matColStart[ele] + row];
      dot += matData[row_idx] * vec[col_idx];
      ele ++; 
    }
...
static void spmvJDS(float *out, int *matColStart, int *matCols,
  ...
  dim3 dimGrid(ceil(dim/(256.0)), 1, 1);
